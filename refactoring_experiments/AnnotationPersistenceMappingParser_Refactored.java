...
public class AnnotationPersistenceMappingParser
    ...
    // =========================================================================
    // REFACTORING: Strutture Dati e Interfacce
    // =========================================================================

    @FunctionalInterface
    private interface AnnotationHandler {
        boolean handle(FieldMapping fm, AnnotatedElement el, Annotation anno, MappingTag tag);
    }

    /**
     * Lista ordinata dei gruppi di gestione.
     * Ogni lambda delega a un metodo specifico per quel dominio funzionale.
     */
    private final List<AnnotationHandler> _memberHandlers = Arrays.asList(
            (fm, el, a, t) -> parseOverrideAnnotations(fm, a, t),
            (fm, el, a, t) -> parseJoinAnnotations(fm, a, t),
            (fm, el, a, t) -> parseBasicColumnAnnotations(fm, a, t),
            (fm, el, a, t) -> parseElementAnnotations(fm, a, t),
            (fm, el, a, t) -> parseKeyAnnotations(fm, a, t),      // Ora suddiviso internamente
            (fm, el, a, t) -> parseStrategyAnnotations(fm, a, t),
            (fm, el, a, t) -> parseIdentityAnnotations(fm, el, a, t)
    );

    // =========================================================================
    // REFACTORING: Metodo Principale
    // =========================================================================
    ...
    protected void parseMemberMappingAnnotations(FieldMetaData fmd) {
        ...
        for (Annotation anno : el.getDeclaredAnnotations()) {
            ...
            // Itera sui gestori: il primo che restituisce true interrompe il ciclo per questa annotazione
            for (AnnotationHandler handler : _memberHandlers) {
                if (handler.handle(fm, el, anno, tag)) {
                    handled = true;
                    break;
                }
            }
    ...
    // =========================================================================
    // REFACTORING: Gruppi Logici (Handlers)
    // =========================================================================

    // --- 1. Overrides (Association, Attribute, Mapping) ---
    private boolean parseOverrideAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case ASSOC_OVERRIDE:    parseAssociationOverrides(fm, (AssociationOverride) anno); return true;
            case ASSOC_OVERRIDES:   parseAssociationOverrides(fm, ((AssociationOverrides) anno).value()); return true;
            case ATTR_OVERRIDE:     parseAttributeOverrides(fm, (AttributeOverride) anno); return true;
            case ATTR_OVERRIDES:    parseAttributeOverrides(fm, ((AttributeOverrides) anno).value()); return true;
            default: return false;
        }
    }

    // --- 2. Join e Tabelle ---
    private boolean parseJoinAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case JOIN_COL:          parseJoinColumns(fm, fm.getValueInfo(), true, (JoinColumn) anno); return true;
            case JOIN_COLS:         parseJoinColumns(fm, fm.getValueInfo(), true, ((JoinColumns) anno).value()); return true;
            case JOIN_TABLE:        parseJoinTable(fm, (JoinTable) anno); return true;
            case X_JOIN_COL:        parseXJoinColumns(fm, fm.getValueInfo(), true, (XJoinColumn) anno); return true;
            case X_JOIN_COLS:       parseXJoinColumns(fm, fm.getValueInfo(), true, ((XJoinColumns) anno).value()); return true;
            case CONTAINER_TABLE:   parseContainerTable(fm, (ContainerTable) anno); return true;
            case COLLECTION_TABLE:  parseCollectionTable(fm, (CollectionTable) anno); return true;
            default: return false;
        }
    }

    // --- 3. Colonne Base, Enum, Temporal, Embedded ---
    private boolean parseBasicColumnAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case COL:               parseColumns(fm, (javax.persistence.Column) anno); return true;
            case COLS:              parseColumns(fm, ((Columns) anno).value()); return true;
            case ENUMERATED:        parseEnumerated(fm, (Enumerated) anno); return true;
            case TEMPORAL:          parseTemporal(fm, (Temporal) anno); return true;
            case ORDER_COLUMN:      parseJavaxOrderColumn(fm, (javax.persistence.OrderColumn) anno); return true;
            case ORDER_COL:         parseOrderColumn(fm, (OrderColumn) anno); return true;
            case EMBEDDED_MAPPING:  parseEmbeddedMapping(fm, (EmbeddedMapping) anno); return true;
            case X_EMBEDDED_MAPPING:
                parseXEmbeddedMappingHelper(fm, (XEmbeddedMapping) anno);
                return true;
            default: return false;
        }
    }

    // --- 4. Element Annotations (Collezioni) ---
    private boolean parseElementAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case ELEM_COL:          parseElementColumns(fm, (ElementColumn) anno); return true;
            case ELEM_COLS:         parseElementColumns(fm, ((ElementColumns) anno).value()); return true;
            case ELEM_JOIN_COL:     parseElementJoinColumns(fm, (ElementJoinColumn) anno); return true;
            case ELEM_JOIN_COLS:    parseElementJoinColumns(fm, ((ElementJoinColumns) anno).value()); return true;
            case ELEM_INDEX:
            ...
            case ELEM_FK:
                ...
                parseForeignKey(fm.getElementMapping().getValueInfo(), efk.name(), efk.enabled(), efk.deferred(), efk.deleteAction(), efk.updateAction());
                return true;
            case ELEM_STRAT:        fm.getElementMapping().getValueInfo().setStrategy(((ElementStrategy) anno).value()); return true;
            case ELEM_CLASS_CRIT:   fm.getElementMapping().getValueInfo().setUseClassCriteria(((ElementClassCriteria) anno).value()); return true;
            case ELEM_NONPOLY:      fm.getElementMapping().setPolymorphic(toPolymorphicConstant(((ElementNonpolymorphic) anno).value())); return true;
            case ELEM_EMBEDDED_MAPPING:
                parseElementEmbeddedMappingHelper(fm, (ElementEmbeddedMapping) anno);
                return true;
            default: return false;
        }
    }

    // --- 5. Key Annotations (Delegator) ---
    private boolean parseKeyAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        // Delega a sotto-metodi per ridurre la complessità
        return parseMapKeyAnnotations(fm, anno, tag) ||
                parseStandardKeyColumns(fm, anno, tag) ||
                parseKeyAdvancedConfig(fm, anno, tag);
    }

    // 5.a MapKey Specifica
    private boolean parseMapKeyAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case MAP_KEY_COL:       parseMapKeyColumn(fm, (MapKeyColumn) anno); return true;
            case MAP_KEY_ENUMERATED: parseMapKeyEnumerated(fm, (MapKeyEnumerated) anno); return true;
            case MAP_KEY_TEMPORAL:  parseMapKeyTemporal(fm, (MapKeyTemporal) anno); return true;
            case MAP_KEY_JOIN_COL:  parseMapKeyJoinColumns(fm, (MapKeyJoinColumn) anno); return true;
            case MAP_KEY_JOIN_COLS: parseMapKeyJoinColumns(fm, ((MapKeyJoinColumns) anno).value()); return true;
            default: return false;
        }
    }

    // 5.b Key Standard
    private boolean parseStandardKeyColumns(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case KEY_COL:           parseKeyColumns(fm, (KeyColumn) anno); return true;
            case KEY_COLS:          parseKeyColumns(fm, ((KeyColumns) anno).value()); return true;
            case KEY_JOIN_COL:      parseKeyJoinColumns(fm, (KeyJoinColumn) anno); return true;
            case KEY_JOIN_COLS:     parseKeyJoinColumns(fm, ((KeyJoinColumns) anno).value()); return true;
            default: return false;
        }
    }

    // 5.c Key Avanzate
    private boolean parseKeyAdvancedConfig(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case KEY_STRAT:         fm.getKeyMapping().getValueInfo().setStrategy(((KeyStrategy) anno).value()); return true;
            case KEY_CLASS_CRIT:    fm.getKeyMapping().getValueInfo().setUseClassCriteria(((KeyClassCriteria) anno).value()); return true;
            case KEY_NONPOLY:       fm.getKeyMapping().setPolymorphic(toPolymorphicConstant(((KeyNonpolymorphic) anno).value())); return true;
            case KEY_FK:
                KeyForeignKey kfk = (KeyForeignKey) anno;
                parseForeignKey(fm.getKeyMapping().getValueInfo(), kfk.name(), kfk.enabled(), kfk.deferred(), kfk.deleteAction(), kfk.updateAction());
                return true;
            case KEY_INDEX:
            ...
            case KEY_EMBEDDED_MAPPING:
                parseKeyEmbeddedMappingHelper(fm, (KeyEmbeddedMapping) anno);
                return true;
            default: return false;
        }
    }

    // --- 6. Strategie e Fetching ---
    private boolean parseStrategyAnnotations(FieldMapping fm, Annotation anno, MappingTag tag) {
        switch (tag) {
            case STRAT:             fm.getMappingInfo().setStrategy(((Strategy) anno).value()); return true;
            case EAGER_FETCH_MODE:  fm.setEagerFetchMode(toEagerFetchModeConstant(((EagerFetchMode) anno).value())); return true;
            case CLASS_CRIT:        fm.getValueInfo().setUseClassCriteria(((ClassCriteria) anno).value()); return true;
            case NONPOLY:           fm.setPolymorphic(toPolymorphicConstant(((Nonpolymorphic) anno).value())); return true;
            default: return false;
        }
    }

    // --- 7. Identità e Constraint ---
    private boolean parseIdentityAnnotations(FieldMapping fm, AnnotatedElement el, Annotation anno, MappingTag tag) {
        switch (tag) {
            case PK_JOIN_COL:       parsePrimaryKeyJoinColumns(fm, (PrimaryKeyJoinColumn) anno); return true;
            case PK_JOIN_COLS:      parsePrimaryKeyJoinColumns(fm, ((PrimaryKeyJoinColumns) anno).value()); return true;
            case TABLE_GEN:         parseTableGenerator(el, (TableGenerator) anno); return true;
            case FK:                parseForeignKey(fm.getValueInfo(), (ForeignKey) anno); return true;
            case INDEX:             parseIndex(fm.getValueInfo(), (Index) anno); return true;
            case UNIQUE:            parseUnique(fm, (org.apache.openjpa.persistence.jdbc.Unique) anno); return true;
            default: return false;
        }
    }

    // =========================================================================
    // Helpers per pulizia logica inline (evita creazione oggetti nello switch)
    // =========================================================================

    private void parseXEmbeddedMappingHelper(FieldMapping fm, XEmbeddedMapping embed) {
        parseEmbeddedMapping(fm,
                DBIdentifier.newColumn(embed.nullIndicatorColumnName(), delimit()),
                DBIdentifier.newConstant(embed.nullIndicatorAttributeName()),
                embed.overrides());
    }

    private void parseKeyEmbeddedMappingHelper(FieldMapping fm, KeyEmbeddedMapping kembed) {
    ...
    private void parseElementEmbeddedMappingHelper(FieldMapping fm, ElementEmbeddedMapping ee) {
